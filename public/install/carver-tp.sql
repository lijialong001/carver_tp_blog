...
CREATE TABLE `carver_article` (
  ...
  `article_guide` int NOT NULL COMMENT '文章导航',
  `article_label` varchar(200) NOT NULL COMMENT '文章标签',
  `click_num` int NOT NULL COMMENT '点击量',
  `is_show` int NOT NULL COMMENT '是否显示',
  `is_top_show` int NOT NULL COMMENT '是否置顶',
  `article_author` varchar(30) NOT NULL DEFAULT '李佳龙' COMMENT '文章作者',
...
) ENGINE=InnoDB AUTO_INCREMENT=120 DEFAULT CHARSET=utf8mb3 COMMENT='文章表';
...
CREATE TABLE `carver_auth_new` (
  ...
  `auth_status` smallint NOT NULL COMMENT '当前使用状态 0 禁用 1 启用',
  `create_time` bigint NOT NULL DEFAULT '0' COMMENT '创建时间',
  `update_time` bigint NOT NULL DEFAULT '0',
  `delete_time` bigint NOT NULL DEFAULT '0',
  PRIMARY KEY (`auth_id`)
) ENGINE=InnoDB AUTO_INCREMENT=23 DEFAULT CHARSET=utf8mb3 COMMENT='权限新表';
...
) ENGINE=InnoDB AUTO_INCREMENT=15 DEFAULT CHARSET=utf8mb3 COMMENT='前台轮播图表';
...
CREATE TABLE `carver_comment` (
  ...
  `create_time` bigint DEFAULT NULL COMMENT '评论时间',
  `delete_time` bigint DEFAULT NULL COMMENT '删除时间',
  PRIMARY KEY (`comment_id`)
) ENGINE=InnoDB AUTO_INCREMENT=12 DEFAULT CHARSET=utf8mb3 COMMENT='评论主表';
...
CREATE TABLE `carver_comment_to_others` (
  ...
  `comment_user_id` int DEFAULT NULL COMMENT '回复的用户id',
  `comment_id` int DEFAULT NULL COMMENT '评论id',
  `comment_to_content` longtext COMMENT '评论内容',
  `article_id` int DEFAULT NULL COMMENT '回复的文章id',
  `comment_to_user_id` int DEFAULT NULL COMMENT '被回复的用户id',
  `create_time` bigint DEFAULT NULL COMMENT '回复时间',
  `delete_time` bigint DEFAULT NULL COMMENT '删除时间',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=7 DEFAULT CHARSET=utf8mb3 COMMENT='评论附表';
...
CREATE TABLE `carver_link` (
  ...
  `is_confirm` int NOT NULL COMMENT '0 待审核  1 审核中  2 审核通过   3 审核不通过',
  `is_doc` tinyint(1) NOT NULL DEFAULT '1' COMMENT '0 在线文档 1 优秀博主链接',
...
CREATE TABLE `carver_navigation` (
  ...
  `p_id` int DEFAULT NULL,
  `is_show` smallint DEFAULT NULL COMMENT '是否显示',
  `create_time` bigint DEFAULT NULL,
  `update_time` bigint DEFAULT NULL,
  `delete_time` bigint DEFAULT NULL,
  PRIMARY KEY (`nav_id`)
) ENGINE=InnoDB AUTO_INCREMENT=18 DEFAULT CHARSET=utf8mb3 COMMENT='前台导航表';
...
) ENGINE=InnoDB AUTO_INCREMENT=13 DEFAULT CHARSET=utf8mb3 COMMENT='公告表';
...
CREATE TABLE `carver_role` (
  ...
  `current_status` smallint NOT NULL DEFAULT '1' COMMENT '0 禁用 1启用',
  `create_time` bigint NOT NULL DEFAULT '0' COMMENT '创建时间',
  `update_time` bigint NOT NULL DEFAULT '0' COMMENT '修改时间',
  `delete_time` bigint NOT NULL DEFAULT '0' COMMENT '删除时间',
  PRIMARY KEY (`role_id`)
) ENGINE=InnoDB AUTO_INCREMENT=82 DEFAULT CHARSET=utf8mb3 COMMENT='角色表';
...
CREATE TABLE `carver_role_auth` (
  ...
  `current_status` smallint NOT NULL COMMENT '0 禁用 1 启用',
  `create_time` bigint NOT NULL DEFAULT '0' COMMENT '创建时间',
  `delete_time` bigint NOT NULL COMMENT '清除时间',
  `update_time` bigint NOT NULL DEFAULT '0' COMMENT '修改时间',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=405 DEFAULT CHARSET=utf8mb3 COMMENT='角色权限表';
...
CREATE TABLE `carver_system_log` (
  ...
  `action_user_id` int DEFAULT NULL COMMENT '用户id',
...
CREATE TABLE `carver_user` (
  ...
  `is_lock` tinyint(1) NOT NULL DEFAULT '0' COMMENT '是否被锁定 1 是  0 否【默认】',
...
CREATE TABLE `carver_user_click` (
  `click_id` int NOT NULL AUTO_INCREMENT COMMENT '点击的id',
  `user_id` int DEFAULT NULL COMMENT '用户id',
  `article_id` int DEFAULT NULL COMMENT '文章id',
  `click_time` bigint DEFAULT NULL COMMENT '上一次点击的时间',
  `create_time` bigint DEFAULT NULL COMMENT '创建时间',
  `update_time` bigint DEFAULT NULL COMMENT '修改时间',
