<?php /*a:3:{s:77:"/www/wwwroot/101.200.121.249/carver_tp_blog/app/admin/view/link/linkList.html";i:1623748047;s:75:"/www/wwwroot/101.200.121.249/carver_tp_blog/app/admin/view/index/index.html";i:1629028550;s:70:"/www/wwwroot/101.200.121.249/carver_tp_blog/app/admin/view/header.html";i:1618120768;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
...
<table class="table table-hover  table-bordered text-center" style="margin-top: 32px;margin-left: 15px;width: 98%">
    <caption class="btn-primary" style="padding-left: 30px;height: 50px;padding-top: 14px;font-size: 18px"><i class="layui-icon layui-icon-link" style="font-size: 19px; color: white;"></i>&nbsp;&nbsp;友情管理 / 友情列表</caption>
    <thead>
    <tr>
        <th class="text-center">友情名称</th>
    ...
    <tbody>
    <?php if(empty($linkList) || (($linkList instanceof \think\Collection || $linkList instanceof \think\Paginator ) && $linkList->isEmpty())): ?>
    <tr>
        <td colspan="4">空如也也</td>
    </tr>
    <?php else: foreach($linkList as $key => $value): ?>
    <tr>
        <td><?php echo htmlentities($value['link_name']); ?></td>
        <td><?php echo htmlentities($value['link_site']); ?></td>
        <td class="is_status">
            <?php switch($value['is_confirm']): case "0": ?> <span style="color:red;"><b>未审核</b></span><?php break; case "1": ?> <span style="color:blue;"><b>正在审核</b></span><?php break; case "2": ?> <span style="color:green;"><b>审核通过</b></span><?php break; case "3": ?> <span style="color: red;"><b>审核不通过</b></span><?php break; ?>
            <?php endswitch; ?>
        </td>
        <td>
            <a href="javascript:void (0)" data-url="<?php echo url('link/upLink'); ?>"
               data-id="<?php echo htmlentities($value['link_id']); ?>"  onclick="upLink(this)">
            <span class="glyphicon glyphicon-edit" style="color: rgb(255, 3, 0);font-size: 23px"><font
...
<span style="float: right;margin-right: 46%"><?php echo $linkList; ?></span>
<script>
     function upLink(that) {
         ...
         layui.use('layer',function () {
             ...
             layer.open({
                 type:2,
                 title:'修改申请链接<span style="color: red">(注:只能修改状态)</span>',
                 area:['300px','300px'],
                 closeBtn:2,
                 content:url+"?id="+id,
             })
