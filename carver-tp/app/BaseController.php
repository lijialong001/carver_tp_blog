...
abstract class BaseController
{
    ...
    public function judgeAuth($controller, $action, $data)
    {
        foreach ($data as $key => $value) {
            $isHasModuleAuth = Db::name("carver_auth_new")->where("auth_name", $controller)->findOrEmpty();
            if ($isHasModuleAuth) {//模块权限
                $isHasActionAuth = Db::name("carver_auth_new")->where("auth_name", $action)->where('p_id', '<>', 0)->findOrEmpty();
                if ($isHasActionAuth) {//方法权限
